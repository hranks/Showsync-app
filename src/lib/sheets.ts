  ...
  try {
    ...
    if (eventsRes.ok) {
      ...
      events = rows.map((row: any[]) => {
        ...
        if (row[1]) {
          const dateStr = String(row[1]).trim().split('T')[0];
          ...
          if (parts.length === 3) {
            ...
            if (parts[0].length === 4) {
              ...
              if (p0 > 12) {
                parsedDate = new Date(p2, p1 - 1, p0, 12, 0, 0);
              } else if (p1 > 12) {
                parsedDate = new Date(p2, p0 - 1, p1, 12, 0, 0);
              } else {
              ...
              // Fallback
              const stdParsed = new Date(cleanStr);
              if (!isNaN(stdParsed.getTime())) {
                parsedDate = new Date(stdParsed.getFullYear(), stdParsed.getMonth(), stdParsed.getDate(), 12, 0, 0);
              }
            }
          } else {
            const stdParsed = new Date(cleanStr);
        ...
        if (isNaN(parsedDate.getTime())) {
          const now = new Date();
          parsedDate = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 12, 0, 0);
        }
